...
def clean_movies_df(movies_df, plots_df):
    ...
    movies_clean_df["languages"] = movies_clean_df["languages"].str.replace(
        '"/m/\w*": ', "", regex=True
    )
    movies_clean_df["country"] = movies_clean_df["country"].str.replace(
        '"/m/\w*": ', "", regex=True
    )
    movies_clean_df["genres"] = movies_clean_df["genres"].str.replace(
        '"/m/\w*": ', "", regex=True
    )
    ...
    movies_clean_df["languages"] = movies_clean_df["languages"].replace(
        "{}", None, regex=False
    )
    movies_clean_df["country"] = movies_clean_df["country"].replace(
        "{}", None, regex=False
    )
    movies_clean_df["genres"] = movies_clean_df["genres"].replace(
        "{}", None, regex=False
    )
    ...
    movies_clean_df["languages"] = movies_clean_df["languages"].str.replace(
        "Language", "", regex=False, case=False
    )

    # merge movies_df and plots_df
    movies_clean_df = movies_clean_df.merge(
        plots_df, on="wiki_id", how="outer"
    )  # from 81741 to 81840

    # save only th the year in released date column and convert it to int object
    movies_clean_df["release_date"] = movies_clean_df["release_date"].str.replace(
        "-.*", "", regex=True
    )
...
def missing_movies_data_check(movies_clean_df):
    for columns in movies_clean_df.columns:
        ...
        print(
            f"{missing_data} out of {len(movies_clean_df[columns])} movies have no {columns} data, which means {missing_data/len(movies_clean_df[columns])*100:.3f}% of the data is missing"
        )
...
def missing_characters_data_check(characters_clean_df):
    for columns in characters_clean_df.columns:
        ...
        print(
            f"{missing_data} out of {len(characters_clean_df[columns])} characters have no {columns} data, which means {missing_data/len(characters_clean_df[columns])*100:.3f}% of the data is missing"
        )
